...
- 기기 간 동기화: Microsoft 계정의 OneDrive 앱 전용 폴더를 이용한 자동 병합, 날짜별 최신 아웃핏 반영, 조건부 재시도
...
- 옷장·룩북·위시리스트·스크랩은 항목 ID별로 병합하며, 다른 항목을 각각 추가한 경우 모두 유지합니다.
- 옷을 삭제하는 동안 다른 기기에서 같은 옷을 수정했다면 수정본을 보존하고, 변경되지 않은 옷의 명시적 삭제만 반영합니다.
- 아웃핏과 메모는 날짜별로 묶어 가장 최근 변경사항을 적용합니다.
- 동시에 업로드한 경우 OneDrive의 최신 eTag를 다시 확인하고 최대 3회 자동 재병합합니다.
