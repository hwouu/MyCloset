...
function CompactCalendar({ cursor, onCursorChange, selectedDate, onSelectDate, outfits = {}, className = "", id, role, ariaLabel = "날짜 선택" }) {
  ...
  return <section className={`compact-calendar${className ? ` ${className}` : ""}`} id={id} role={role} aria-label={ariaLabel} onKeyDown={(event) => { if (event.key === "Escape" && pickerOpen) { event.preventDefault(); event.stopPropagation(); setPickerOpen(false); } }}>
    ...
    <div className="compact-calendar-grid"><div className="calendar-grid weekday-row">{WEEKDAYS.map((day, index) => <div className={index === 0 ? "sunday" : ""} key={day}>{day}</div>)}</div><div className="calendar-grid month-grid">{days.map(({ date, key, current }) => { const hasOutfit = (outfits[key] ?? []).length > 0; return <button type="button" className={`day-cell${current ? "" : " outside"}${key === selectedDate ? " selected" : ""}${hasOutfit ? " has-outfit" : ""}`} key={key} onClick={() => onSelectDate(key)} aria-label={`${key}${hasOutfit ? ", 아웃핏 있음" : ", 아웃핏 없음"}`} title={hasOutfit ? "아웃핏 있음" : "아웃핏 없음"}><span className="day-number-row"><span className="day-number">{date.getDate()}</span></span>{hasOutfit && <span className="outfit-indicator" aria-hidden="true"/>}</button>; })}</div></div>
  </section>;
}
...
export function App() {
  ...
  const [outfitTransferMode, setOutfitTransferMode] = useState("copy");
  const [outfitTransferDate, setOutfitTransferDate] = useState(TODAY);
  const [outfitTransferCursor, setOutfitTransferCursor] = useState(() => { const today = new Date(); return new Date(today.getFullYear(), today.getMonth(), 1); });
  ...
  const [dateTransitionDirection, setDateTransitionDirection] = useState("");
  ...
  const outfitSearchRef = useRef(null);
  ...
  const moveSelectedDate = (offset) => {
    ...
    setDateTransitionDirection(offset < 0 ? "previous" : "next");
    setSelectedDate(nextDateKey);
    setCursor(new Date(nextDate.getFullYear(), nextDate.getMonth(), 1));
    setCalendarOpen(false);
  };
  const chooseSelectedDate = (date) => {
    if (date !== selectedDate) setDateTransitionDirection(date < selectedDate ? "previous" : "next");
  ...
  const openOutfitTransfer = (mode) => {
    const initialDate = new Date(`${selectedDate}T12:00:00`);
    initialDate.setDate(initialDate.getDate() + 1);
    setOutfitTransferMode(mode);
    setOutfitTransferDate(iso(initialDate));
    setOutfitTransferCursor(new Date(initialDate.getFullYear(), initialDate.getMonth(), 1));
    setModal("outfitTransfer");
  };
  const applyOutfitTransfer = () => {
    if (!draftOutfit.length || !outfitTransferDate || outfitTransferDate === selectedDate) return;
    const sourceDate = selectedDate;
    const targetDate = outfitTransferDate;
    setOutfits((current) => transferOutfit(current, sourceDate, targetDate, draftOutfit, outfitTransferMode));
    const date = new Date(`${targetDate}T12:00:00`);
    setSelectedDate(targetDate);
    setCursor(new Date(date.getFullYear(), date.getMonth(), 1));
    closeModal();
    setNotice(`${sourceDate.replaceAll("-", ". ")} 아웃핏을 ${targetDate.replaceAll("-", ". ")}로 ${outfitTransferMode === "move" ? "이동" : "복사"}했어요.`);
  };
  const openLookbookCreator = ({ itemIds = [], sourceDate = "" } = {}) => {
    setEditingLookbookId(null);
    setDraftLookbookIds([...itemIds]);
    setLookbookName(sourceDate ? `${sourceDate.replaceAll("-", ". ")} 아웃핏` : nextLookbookName(lookbooks));
  ...
  const saveLookbook = () => {
    ...
    closeModal();
    setNotice(editingLookbookId ? "룩북을 수정했어요." : "룩북에 새 아웃핏을 저장했어요.");
  };
  ...
  const addItemToSelectedOutfit = (id) => {
    ...
    if (current.includes(id)) { setNotice("이미 오늘의 아웃핏에 있는 옷이에요."); return; }
    setOutfits((existing) => ({ ...existing, [selectedDate]: [...(existing[selectedDate] ?? []), id] }));
    setNotice(`${itemMap[id].name}을(를) ${selectedDateLabel} 아웃핏에 추가했어요.`);
  };
  const removeItemFromSelectedOutfit = (id) => {
    ...
    setNotice(`${itemMap[id]?.name ?? "옷"}을(를) 아웃핏에서 뺐어요.`);
  };
  ...
  const startOutfitReorder = (event, id) => {
    ...
    const onUp = () => {
      if (drag.active) setNotice("아웃핏 순서를 변경했어요.");
  ...
  const saveOutfit = () => { setOutfits((current) => ({ ...current, [selectedDate]: draftOutfit })); closeModal(); setNotice("아웃핏을 저장했어요."); };
  ...
  const deleteOutfit = () => {
    ...
    closeModal(); setNotice("이 날짜의 아웃핏을 삭제했어요.");
  };
  ...
  useEffect(() => {
    const onShortcut = (event) => {
      if (!window.matchMedia("(min-width: 901px)").matches || event.defaultPrevented) return;
      ...
      if (event.key === "Escape") {
        ...
        else if (calendarOpen) { event.preventDefault(); setCalendarOpen(false); }
        return;
      }
      if (isTyping || modal || event.metaKey || event.ctrlKey || event.altKey) return;
      ...
      if (event.key === "/" || event.code === "Slash") {
        const searchInput = {
          calendar: outfitSearchRef,
          closet: wardrobeSearchRef,
          lookbook: lookbookSearchRef,
          wishlist: wishlistSearchRef,
        }[view]?.current;
        if (searchInput) {
          event.preventDefault();
          searchInput.focus();
      ...
      if (event.code === "KeyN") {
        if (view === "calendar" || view === "closet") { event.preventDefault(); openNewItem(); }
        else if (view === "lookbook") {
          event.preventDefault();
          if (items.length) openLookbookCreator();
          else setNotice("룩북을 만들려면 먼저 옷을 등록해주세요.");
        } else if (view === "wishlist") { event.preventDefault(); openNewWish(); }
        return;
      }
      if (view !== "calendar") return;
      if (event.code === "KeyT") {
        event.preventDefault();
        chooseSelectedDate(TODAY);
      } else if (event.code === "KeyC") {
        event.preventDefault();
        setCalendarOpen((current) => !current);
      } else if (event.key === "ArrowLeft" || event.key === "ArrowRight") {
        event.preventDefault();
        moveSelectedDate(event.key === "ArrowLeft" ? -1 : 1);
      }
    };
  ...
  return <div className={`app-shell${sidebarCollapsed ? " sidebar-collapsed" : ""}`}>
    <aside className="sidebar">
      ...
      <nav className="main-nav">
        <button className={view === "calendar" ? "active" : ""} onClick={() => setView("calendar")} aria-label="아웃핏" title="아웃핏"><CalendarDots size={22}/><span>아웃핏</span></button>
    ...
    {view === "calendar" && <main className="calendar-page closet-first-page" style={{ "--detail-width": `${detailWidth}px`, "--dense-detail-width": `${Math.round(detailWidth * DESKTOP_UI_DENSITY)}px` }}>
      ...
      <section className="calendar-main outfit-main">
        <header className="outfit-main-header"><div className="outfit-heading-copy"><span className="mobile-section-label">STEP 1</span><h1>오늘의 아웃핏</h1><div className="outfit-date-line"><p key={selectedDate} className={`outfit-date-value${dateTransitionDirection ? ` date-${dateTransitionDirection}` : ""}`}>{selectedDateLabel}</p>{selectedDate === TODAY && <span className="today-date-badge">Today</span>}</div><small className="mobile-outfit-help">{items.length ? "옷장에서 아이템을 골라 오늘 입을 조합을 만들어보세요." : "먼저 옷 한 벌을 등록하면 아웃핏을 시작할 수 있어요."}</small></div><div className="outfit-date-quick-nav" role="group" aria-label="아웃핏 날짜 빠른 이동"><button type="button" onClick={() => moveSelectedDate(-1)} aria-label="이전 날짜로 이동" title="이전 날짜 (←)"><CaretLeft size={18}/></button><button type="button" onClick={() => moveSelectedDate(1)} aria-label="다음 날짜로 이동" title="다음 날짜 (→)"><CaretRight size={18}/></button></div></header>
        <section key={selectedDate} className={`today-outfit outfit-workspace${dateTransitionDirection ? ` date-${dateTransitionDirection}` : ""}${dropActive ? " drop-active" : ""}${items.length ? "" : " is-empty-wardrobe"}`} aria-label="오늘의 아웃핏 드롭 영역">
      ...
      <aside className="detail-rail calendar-detail-rail closet-first-rail">
        {calendarOpen && <CompactCalendar className="rail-calendar" id="outfit-calendar-popover" role="dialog" ariaLabel="아웃핏 날짜 선택" cursor={cursor} onCursorChange={setCursor} selectedDate={selectedDate} onSelectDate={chooseSelectedDate} outfits={outfits}/>}
      </aside>
      <button className={`calendar-fab${calendarOpen ? " is-open" : ""}`} type="button" onClick={() => setCalendarOpen((current) => !current)} onKeyDown={(event) => { if (event.key === "Escape") setCalendarOpen(false); }} aria-label={calendarOpen ? "달력 닫기" : "달력 열기"} aria-expanded={calendarOpen} aria-controls="outfit-calendar-popover" title={`${calendarOpen ? "달력 닫기" : "달력 열기"} (C)`}>{calendarOpen ? <X size={22} weight="bold"/> : <CalendarDots size={24}/>}</button>
    </main>}
    ...
    {modal === "shortcuts" && <Modal title="키보드 단축키" subtitle="PC에서 자주 쓰는 기능을 빠르게 실행하세요" eyebrow="" onClose={closeModal} compact className="shortcut-modal"><div className="shortcut-list"><div><kbd>/</kbd><span>현재 탭 검색창으로 이동</span></div><div><kbd>N</kbd><span>현재 탭에서 새 항목 등록</span></div><div><span className="shortcut-key-group"><kbd>←</kbd><kbd>→</kbd></span><span>아웃핏 날짜를 이전·다음 날로 이동</span></div><div><kbd>T</kbd><span>아웃핏 날짜를 오늘로 이동</span></div><div><kbd>C</kbd><span>아웃핏 달력 열기·닫기</span></div><div><kbd>Esc</kbd><span>검색 포커스·열린 창·달력·메뉴 닫기</span></div><div><kbd>?</kbd><span>이 단축키 안내 열기</span></div></div><footer className="modal-actions"><button type="button" className="primary-button compact" onClick={closeModal}>확인</button></footer></Modal>}
